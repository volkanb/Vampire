...
public class vp_FPWeaponMeleeAttack : vp_Component
{
					...
					/*if(hit.collider.transform.parent)
					{
						if(hit.collider.transform.parent.parent)
						{
							if((gameObject.transform.parent.parent.parent.tag=="Slayer") && (Player.CurrentWeaponName.Get()=="4Mace") )
							{
								...
								{
									if((hit.collider.transform.parent.parent.GetComponent<AIPathVampire>().isDown==true) && (hit.collider.GetComponent<vp_DamageHandler2>().m_CurrentHealth<=1))
									{
										hit.collider.transform.parent.parent.GetComponent<AIPathVampire>().isDead=true;
									}
								}
	...
	void ApplyDamage(RaycastHit hit)
	{
		...
		if (body != null && !body.isKinematic && gameObject.transform.parent.parent.tag!="Vampire" && gameObject.transform.parent.parent.tag!="Slayer" 
		    && gameObject.transform.parent.parent.tag!="Human" && gameObject.transform.parent.parent.tag!="PossessedHuman" && gameObject.transform.parent.parent.tag!="ArmedHuman")
